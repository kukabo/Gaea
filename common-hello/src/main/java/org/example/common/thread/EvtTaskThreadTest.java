...
public class EvtTaskThreadTest {
    ...
    static class DatabaseSimulator {
        ...
        public void insertMasterData() {
            createMasterRecord("A", preStatus, "A1-异步", "A2", "A3");
            createMasterRecord("B", preStatus, "B1-异步", "B2", "B3");
            createMasterRecord("C", preStatus, "C1-异步", "C2", "C3");
        }
    ...
    static class BusinessProcessor {
        ...
        private void initializeTaskQueues(List<MasterRecord> masterList) {
            for (MasterRecord master : masterList) {
                if (master.slaveTasks.isEmpty()) {
                    System.out.printf("主业务%s无子业务%n", master.name);
                    return;
                }
        ...
        private void initBizQueueMapAsync(MasterRecord master) {
            ...
            System.out.printf("[%s] 业务 %s 异步子任务初始化 %d 个子任务%n",
        ...
        private void initBizQueueMapSync(MasterRecord master) {
            ...
            System.out.printf("[%s] 业务 %s 同步子任务初始化 %d 个子任务%n",
        ...
        private void initBizQueueMapAll(MasterRecord master) {
            ...
            if (!masterStatus.equals(preStatus))
                throw new RuntimeException("主业务的初始状态不是待处理" + masterStatus);
            ...
            for (String masterName : aSyncBizQueueMap.keySet()) {
                ...
                while (!aSyncQueue.isEmpty()) {
                    try {
                        ...
                        throw new RuntimeException("处理中断" + masterName, e);
                    }
                }
            }
            //线程处理
            if (!aSyncList.isEmpty())
                executor.submit(() -> processAsyncBizTask(aSyncList));
        }

        /**
         * 业务逻辑
         *
         * @param aSyncSlaveRecords 全部的异步子业务，插入etl实例
         */
        private void processAsyncBizTask(List<SlaveRecord> aSyncSlaveRecords) {
            for (SlaveRecord slaveRecord : aSyncSlaveRecords) {
                ...
                try {
                    ...
                    throw new RuntimeException("中断异常" + name);
                }
                if (!subName.equals("B1-异步")) {
         ...
         * 处理前的校验
         * 1.主业务必须是 处理中
         * 2.要执行的子业务开始状态必须是 待处理，并更新为 处理中
         * 3.全队列的首元素（第一个子业务）必须是 处理中
         */
        private void checkStatusBeforeExecute(SlaveRecord slaveTask) {
            ...
            if (!masterStatus.equals(activeStatus))
                throw new RuntimeException("主业务状态不是处理中，无法执行" +
                        masterStatus + "---" + name);

            if (!subStatus.equals(preStatus))
                throw new RuntimeException("子业务状态不是待处理，无法执行" +
                        subStatus + "---" + name);
            mockDataBase.updateSubTaskStatus(masterName, subName, activeStatus);
            ...
            if (!activeSubName.equals(subName))
                throw new RuntimeException("子业务执行时顺序有误！" +
                        activeSubName + "---" + name);
            if (!activeSubStatus.equals(activeStatus))
                throw new RuntimeException("没有完成子业务激活！" +
         ...
         * 4.否则激活下一个子业务
         */
        private void successTakeCurrentSlaveAndActiveNext(SlaveRecord currentSlaveRecord) {
            String masterName = currentSlaveRecord.masterName;
            String subName = currentSlaveRecord.subName;
            String name = masterName + "---" + subName;
            String currentSlaveSubStatus = currentSlaveRecord.subStatus;
            //1
            if (!currentSlaveSubStatus.equals(activeStatus))
                throw new RuntimeException("当前子业务状态不是处理中，无法更新子业务状态为【处理成功】");
            ...
            try {
                ...
                if (!slaveRecord.subName.equals(subName))
                    throw new RuntimeException("当前子业务不在处理队列中，移除失败");
                System.out.printf("当前子业务从全队列移除成功%s%n", name);
            } catch (InterruptedException e) {
                throw new RuntimeException("队列访问异常" + name, e);
            }
            //3
            ...
            SlaveRecord element = allBizQueue.element();
            if (!element.subStatus.equals(preStatus))
                throw new RuntimeException("全队列下个子业务状态不是待处理，激活失败");
            element.subStatus = activeStatus;
        }
        ...
        private void processSyncBizTask(String masterName) {
            ...
            while (!syncQueue.isEmpty()) {
                ...
                try {
                    ...
                    throw new RuntimeException("处理被中断" + masterName, e);
                }
        ...
        private void executeSyncSlaveTask(SlaveRecord slaveTask) {
            ...
            System.out.printf("[%s] | 业务 %s | 【任务启动】%s%n",
            ...
            try {
                ...
                System.out.printf("[%s] | 业务 %s | 【任务执行】%s | 预计耗时: %d秒%n",
                        currentThread, masterName, slaveName, duration);
                try {
                    TimeUnit.SECONDS.sleep(duration);
                } catch (InterruptedException e) {
                    throw new RuntimeException("任务中断" + name, e);
                }
                //处理后执行成功
                successTakeCurrentSlaveAndActiveNext(slaveTask);
            } catch (Exception e) {
